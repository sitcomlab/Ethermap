...
var db_host = process.env.DB_PORT_5984_TCP_ADDR || 'admin:password@localhost';
...
function getOrCreateDb(name, callback) {
  nano.db.get(name, async function(err, body) {
    if (err && err.statusCode === 404) {
      nano.db.create(name, function(err) {
        callback(nano.use(name));
      });
    } else if (err) {
      console.error(`couldnt get DB: ${err}`)
    } else {
      callback(nano.use(name));
    }
  });
...
function fetchAllRevDocs(db, docId, revs, cb) {
  ...
  const promises = revs.map(rev => db.get(docId, { rev: rev.rev }))
...
module.exports.listStream = function(dbId, options, resultStream) {
  getOrCreateDb(dbId, function(db) {
    ...
    db.listAsStream(params)
      .on('error', err => { resultStream.end(500, err.message) })
      .pipe(resultStream);
  });
};
